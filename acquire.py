...
# Two factors at play here actually, lol
# 1. IF for downmixing
# 2. Sample clock 
...
def acquire(prn, front):
    ...
    for doppler in numpy.arange(DOPP_MIN, DOPP_MAX, DOPP_BIN_WIDTH):
        ...
        carrier_i = (numpy.sign(numpy.cos(2 * math.pi * f_carrier * t))).astype('int32')
        carrier_q = (numpy.sign(numpy.sin(2 * math.pi * f_carrier * t))).astype('int32')
        ...
        for chip_delay in numpy.arange(0, 1023, 0.5):
            ...
            for i in range(NUM_NONCOHERENT_CHUNKS):
                ...
                # Convolutinate (I&Q)
                sum_i = float(numpy.correlate(chunk_carrier_mixed_i, chunk_code))
                sum_q = float(numpy.correlate(chunk_carrier_mixed_q, chunk_code))
